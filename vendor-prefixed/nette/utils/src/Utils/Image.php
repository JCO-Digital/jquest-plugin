...
class Image
{
	 ...
	 * @throws JcoreBroiler\Nette\NotSupportedException if gd extension is not loaded
	 * @throws UnknownImageFileException if file not found or file type is not known
	 */
	public static function fromFile(string $file, ?int &$type = null): static
	{
		if (!extension_loaded('gd')) {
			throw new \JcoreBroiler\Nette\NotSupportedException('PHP extension GD is not loaded.');
		}
	...
	public static function fromBlank(int $width, int $height, ImageColor|array|null $color = null): static
	{
		...
		if ($width < 1 || $height < 1) {
			throw new \JcoreBroiler\Nette\InvalidArgumentException('Image width and height must be greater than zero.');
		}
	...
	public static function typeToExtension(int $type): string
	{
		if (!isset(self::Formats[$type])) {
			throw new \JcoreBroiler\Nette\InvalidArgumentException("Unsupported image type '$type'.");
		}
	...
	public static function extensionToType(string $extension): int
	{
		...
		if (!isset($extensions[$extension])) {
			throw new \JcoreBroiler\Nette\InvalidArgumentException("Unsupported file extension '$extension'.");
		}
	...
	): array
	{
		...
		if ($mode & self::Stretch) { // non-proportional
			if (!$newWidth || !$newHeight) {
				throw new \JcoreBroiler\Nette\InvalidArgumentException('For stretching must be both width and height specified.');
			}
			...
			if (!$newWidth && !$newHeight) {
				throw new \JcoreBroiler\Nette\InvalidArgumentException('At least width or height must be specified.');
			}
	 ...
	 * @throws JcoreBroiler\Nette\MemberAccessException
	 */
	...
	private static function isPercent(int|string &$num): bool
	{
		...
		throw new \JcoreBroiler\Nette\InvalidArgumentException("Expected dimension in int|string, '$num' given.");
	}
	...
	{
		throw new \JcoreBroiler\Nette\NotSupportedException('You cannot serialize or unserialize ' . self::class . ' instances.');
	}
