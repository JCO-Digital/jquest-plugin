...
class Arrays
{
	 ...
	 * @param  ?T  $default
	 * @return ?T
	 * @throws JcoreBroiler\Nette\InvalidArgumentException if item does not exist and default value is not provided
	 */
	public static function get(array $array, string|int|array $key, mixed $default = null): mixed
	{
		foreach (is_array($key) ? $key : [$key] as $k) {
			if (is_array($array) && array_key_exists($k, $array)) {
				...
				if (func_num_args() < 3) {
					throw new \JcoreBroiler\Nette\InvalidArgumentException("Missing item '$k'.");
				}
	 ...
	 * @param  array-key|array-key[]  $key
	 * @return ?T
	 * @throws JcoreBroiler\Nette\InvalidArgumentException if traversed item is not an array
	 */
	public static function &getRef(array &$array, string|int|array $key): mixed
	{
		foreach (is_array($key) ? $key : [$key] as $k) {
			if (is_array($array) || $array === null) {
				...
				throw new \JcoreBroiler\Nette\InvalidArgumentException('Traversed item is not an array.');
			}
		}
	...
	public static function associate(array $array, $path): array|\stdClass
	{
		...
		if (!$parts || $parts === ['->'] || $parts[0] === '=' || $parts[0] === '|') {
			throw new \JcoreBroiler\Nette\InvalidArgumentException("Invalid path '$path'.");
		}
	...
	public static function pick(array &$array, string|int $key, mixed $default = null): mixed
	{
		if (array_key_exists($key, $array)) {
			...
			throw new \JcoreBroiler\Nette\InvalidArgumentException("Missing item '$key'.");
