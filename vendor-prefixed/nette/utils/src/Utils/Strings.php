...
class Strings
{
	use JcoreBroiler\Nette\StaticClass;
	...
	/**
	 * @deprecated use JcoreBroiler\Nette\Utils\Validator::isUnicode()
	 */
	 ...
	 * @throws JcoreBroiler\Nette\InvalidArgumentException if code point is not in valid range
	 */
	public static function chr(int $code): string
	{
		if ($code < 0 || ($code >= 0xD800 && $code <= 0xDFFF) || $code > 0x10FFFF) {
			throw new \JcoreBroiler\Nette\InvalidArgumentException('Code point must be in range 0x0 to 0xD7FF or 0xE000 to 0x10FFFF.');
		} elseif (!extension_loaded('iconv')) {
			throw new \JcoreBroiler\Nette\NotSupportedException(__METHOD__ . '() requires ICONV extension that is not loaded.');
		}
	...
	public static function ord(string $c): int
	{
		...
		if (!$tmp) {
			throw new \JcoreBroiler\Nette\InvalidArgumentException('Invalid UTF-8 character "' . ($c === '' ? '' : '\x' . strtoupper(bin2hex($c))) . '".');
		}
	...
	public static function substring(string $s, int $start, ?int $length = null): string
	{
		if (function_exists('mb_substr')) {
			...
			throw new \JcoreBroiler\Nette\NotSupportedException(__METHOD__ . '() requires extension ICONV or MBSTRING, neither is loaded.');
		} elseif ($length === null) {
	...
	): string
	{
		if (is_object($replacement) || is_array($replacement)) {
			if (!is_callable($replacement, false, $textual)) {
				throw new \JcoreBroiler\Nette\InvalidStateException("Callback '$textual' is not callable.");
			}
